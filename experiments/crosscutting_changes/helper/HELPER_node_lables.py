...
from experiments.crosscutting_changes.helper.HELPER_GENERIC import clean_up_string_ast_literal
...
def remove_certain_info(graph, whatToRemove, withEmbeddings=True): 
    ...
    #for node, data in graph.nodes(data=True):
     #   try: #node_data = eval(data['label'])
      ...
      #      node_data = ast.literal_eval(clean_up_string_ast_literal(data['label']))
       ...
       # node_data.pop('changeType', None)

        #data['label'] = node_data

    # to_remove = set(whatToRemove)
    if not withEmbeddings:
        whatToRemove.discard("embedding")  # keep embedding if requested

    for _, data in graph.nodes(data=True):
        for k in whatToRemove:
            data.pop(k, None)  # remove top-level attributes if present

    return graph
    
def get_Predessors(graph, item_starting_node): 
    isPredessor_nodes = {}
    ...
    for node, data in graph.nodes(data=True):
        ...
        try: #node_data = eval(data['label'])
            ...
            if isPredessor and node != str(item_starting_node):
                isPredessor_nodes[node]=data
...
def compute_change_class(graph, withEmbeddings=True, isPredessor=True):
    ...
    for node, data in graph.nodes(data=True):
        ...
        if withEmbeddings: 
            ...
            if isPredessor:
                ...
                tuple_info = (node, {'label': str(node_data) , 'node_id': node_id,'graph': graph }   )
...
def mark_neightbors(graph):
    ...
    for node, data in graph.nodes(data=True):
        ...
        if change_type != 'Preserve':
                ...
                for s in successors: 
                    flagged_as_successor_of_changed.add(s)
