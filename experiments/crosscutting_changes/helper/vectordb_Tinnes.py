...
class ChangeGraphVectorDB():
    ...
    def _create_documents(self, doc_gen: Iterable, scope: str = 'train') -> List[Document]:
        ...
        for dataset, file_path in doc_gen:
            loader = JSONLoader(
                ...
                metadata_func=partial(self._metadata_func, dataset, scope, file_path)
            )
    ...
    def _metadata_func(self, dataset:str, scope:str, file_path:str ,record: dict, metadata: dict) -> dict:
        """
        ...
        metadata['diff_graph_id'] = record.get("diff_graph_id")
        metadata['graph_prompt'] =record.get("graph_prompt")
        metadata['graph_completion'] = record.get("graph_completion")
        ...
        if "source" in metadata:
            ...
            #define the directory where all results of step 3 are saved and use the info there
            path_obj = Path(file_path)
            dataset_index = path_obj.parts.index(dataset)
            dataset_parent= path_obj.parts[dataset_index - 1]
            source = source[source.index(dataset_parent):]
