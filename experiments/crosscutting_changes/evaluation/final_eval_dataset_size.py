...
from experiments.crosscutting_changes.helper.HELPER_final_eval import add_node_level_ap, load_final_results, prune_leaf_edges
...
def plot_scatter_with_reg(tmp: pd.DataFrame, log_x: bool = True, min_pts: int = 2):
    ...
    for approach, base_color in approach_palette.items():      # CHANGED
        ...
        labels.append(approach)                                # CHANGED

    plt.legend(handles, labels, title="",              # CHANGED
    ...
    plt.savefig("precision_datasets.pdf", bbox_inches="tight", dpi=300)  # ✅ add this line
...
compare_trainset_size(df, comparison="total", metric="prec@{k}")
#compare_testset_size(df)
