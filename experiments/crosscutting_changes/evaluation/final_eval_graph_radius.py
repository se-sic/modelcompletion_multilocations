...
from experiments.crosscutting_changes.evaluation.final_eval_train_statistics import plot_distance_frequency_by_label

from experiments.crosscutting_changes.helper.HELPER_final_eval import add_node_level_ap, compute_precision, flatten_if_nested, flatten_to_1d, load_final_results, prune_leaf_edges
...
def plot_radius_means_lineplot(keys, df_long, metric_col, radii):
    ...
    for idx, k in enumerate(keys):
        ...
        ax.plot(x, y, marker="o", label=f"k={idx+1}", color=palette[idx], linewidth=1.2)
    ...
    ax.plot(radii_str, cum_probs, marker='s', linestyle='--', color='black',             
...
def plot_radi_dependence(keys,df_long, metric_col, radii): 
    ...
    for idx, k in enumerate(keys):                                       
        ...
        for r, m in means_k.items():                                   
            ...
            ax.scatter(x_pos, m,                                       
                       color=palette[idx], s=30, zorder=10,
