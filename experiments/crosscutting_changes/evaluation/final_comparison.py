...
grand_grand_parent = os.path.dirname(grandparent_dir)
sys.path.insert(0, grand_grand_parent)


from experiments.crosscutting_changes.helper.HELPER_final_eval import add_node_level_ap, filter_keys_with_all_approaches, load_final_results, prune_leaf_edges
...
def plot_comparison_box(df):
    ...
    plt.tight_layout()
    plt.savefig("/Users/TODO/Downloads/top5_precision_boxplot.png", bbox_inches="tight", dpi=300)
    plt.show()
...
def plot_comparison_violin_all_overlay(df, metric_tpl="prec@{k}_all", ks=range(1, 11)):  # CHANGED
    ...
    for idx, k in enumerate(ks):  
        ...
        for i, approach in enumerate(overall_order):
            ...
            if not np.isnan(m):                              
                ax.scatter(i, m, s=40, zorder=20, c=[palette[idx]], edgecolors='none', alpha=0.6) # CHANGED)  
                print(f"Mean for approach={approach}, k={k}: {m:.4f}")  # CHANGED
     # ---------- legend outside -------------------------------------------
    #handles, labels = ax.get_legend_handles_labels()
    #by_label = dict(zip(labels, handles))

    #legend_patches = [Patch(facecolor="none", edgecolor=palette[idx], label=f"k={k}")
    #for idx, k in enumerate(ks)]
    ...
    Line2D(
        ...
        markersize=8,
        alpha=0.6,          # makes them slightly faded (blasser)
        linestyle='None',
        label=f"k={k}"
    )
    for idx, k in enumerate(ks)
    ...
    ax.set_ylabel("Probability Density")  # CHANGED     
    labels = [lbl.get_text().replace("NeuralNetwork", "NextFocus") 
    ...
    plt.savefig(
    "../resultsmajorrevision/finaloutputNN/final_comparison_violinplot_major_todo.pdf",
    bbox_inches="tight"
    )
    plt.show()  
...
if __name__ == "__main__":
    ...
    if not tenfold:
        ...
        file_path = "../resultsmajorrevision/finaloutputNNall/"
        df = pd.DataFrame()
        for sub in os.listdir(file_path):
            sub_path = os.path.join(file_path, sub)
            if os.path.isdir(sub_path):
                df_10fold = load_final_results(sub_path)
                df = pd.concat([df, df_10fold], ignore_index=True)
   ...
   # n#euralnet_vs_others_manwid(df, target=  "Random")          
    ...
    #plot_comparison_violin_all_overlay(df, metric_tpl="recall@{k}")
